...
import urllib.parse
import attr
from . import register, Site, SiteException, CloudflareException
...
class FanFictionNet(Site):
    ...
    def extract(self, url):
        ...
        if chapter_select:
            ...
            options = re.findall(
                r'<option.+?value="?(\d+)"?[^>]*>([^<]+)', str(chapter_select)
            )
            for option in options:
                story.add(
                    Chapter(
                        title=option[1],
                        contents=self._chapter(base_url + option[0] + suffix),
                        date=False,
                    )
                )
            ...
            story.add(
                Chapter(title=story.title, contents=self._chapter(url), date=published)
            )
        ...
        if self._cloudflared:
            ...
            try:
                ...
                raise CloudflareException(
                    "Couldn't fetch, presumably because of Cloudflare protection, and falling back to archive.org failed; if some chapters were succeeding, try again?",
                    url,
                    fallback,
                )
        try:
