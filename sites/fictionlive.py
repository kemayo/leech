...
class FictionLive(Site):
    ...
    def extract(self, url):
        ...
        chapters = (
            ({"ct": 0},)
            + tuple(c for c in response["bm"] if not c["title"].startswith("#special"))
            + ({"ct": 9999999999999999},)
        )
        ...
        for prevc, currc, nextc in contextiterate(chapters):
            ...
            updated = currc["ct"]
            for segment in (
                d for d in data if not d.get("t", "").startswith("#special")
            ):
                ...
                try:
                    if segment["nt"] == "chapter":
                        ...
                        logger.info(
                            "Skipped chapter-segment of unhandled type: %s",
                            segment["nt"],
                        )
