...
from . import register, SiteException
...
class XenForo2(XenForo):
    def _base_story(self, soup):
        ...
        return Story(
            ...
            author=soup.find("div", class_="p-description")
    ...
    def _clean_spoilers(self, post, chapterid):
        ...
        for spoiler in post.find_all(class_="bbCodeSpoiler"):
            ...
            if self.options["skip_spoilers"]:
                link = self._footnote(
                    spoiler.find(class_="bbCodeBlock-content").extract(), chapterid
                )
                if spoiler_title:
    ...
    def _post_date(self, post):
        if post.find("time"):
            return datetime.datetime.fromtimestamp(
                int(post.find("time").get("data-time"))
            )
