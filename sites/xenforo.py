...
from . import register, Site, SiteException, SiteSpecificOption
...
class XenForo(Site):
    ...
    def matches(cls, url):
        match = re.match(
            r"^(https?://%s/threads/[^/]*\d+/(?:\d+/)?reader)/?.*" % cls.domain, url
        )
        if match:
    ...
    def extract(self, url):
        ...
        if url.endswith("/reader"):
            ...
            reader_url = (
                soup.find("div", class_="threadmarks-reader").find("a").get("href")
            )
        else:
            reader_url = False
        ...
        if reader_url:
            ...
            marks = [
                mark
                for mark in self._chapter_list(url)
                if "/members" not in mark.get("href")
                and "/threadmarks" not in mark.get("href")
            ]
    ...
    def _base_story(self, soup):
        ...
        return Story(
            ...
            author=soup.find("p", id="pageDescription")
    ...
    def _threadmark_title(self, post):
        ...
        return "".join(
            post.select("div.threadmarker > span.label")[0].findAll(
    ...
    def _chapter_list_threadmarks(self, url):
        ...
        while fetcher:
            ...
            response = self.session.post(
                f"https://{self.domain}/index.php?threads/threadmarks/load-range",
                data={
        ...
        marks = soup.find(class_="threadmarks").select(
            "li.primaryContent.threadmarkListItem a, li.primaryContent.threadmarkItem a"
        )
        if not marks:
    ...
    def _chapter_list_index(self, url):
        ...
        links = post.find("blockquote", class_="messageText").find_all(
            "a", class_="internalLink"
        )
        if not links:
    ...
    def _clean_chapter(self, post, chapterid):
        ...
        for tag in post.select(
            ".quoteExpand, .bbCodeBlock-expandLink, .bbCodeBlock-shrinkLink"
        ):
            tag.decompose()
        self._clean(post)
    ...
    def _clean_spoilers(self, post, chapterid):
        ...
        for spoiler in post.find_all(class_="ToggleTriggerAnchor"):
            ...
            if self.options["skip_spoilers"]:
                link = self._footnote(
                    spoiler.find(class_="SpoilerTarget").extract(), chapterid
                )
                if spoiler_title:
    ...
    def _post_date(self, post):
        ...
        if "title" in maybe_date.attrs:
            ...
            return datetime.datetime.strptime(
                maybe_date["title"], "%b %d, %Y at %I:%M %p"
            )
