...
class Wattpad(Site):
    ...
    def extract(self, url):
        workid = re.match(
            r"^https?://(?:www\.)?wattpad\.com/story/(\d+)?.*", url
        ).group(1)
        info = self.session.get(
            f"https://www.wattpad.com/api/v3/stories/{workid}"
        ).json()
    ...
    def _chapter(self, chapterid):
        ...
        api = self.session.get(
            f"https://www.wattpad.com/apiv2/storytext?id={chapterid}"
        )
