...
import datetime
import re
from . import register, Site, SiteSpecificOption
...
class RoyalRoad(Site):
    ...
    def matches(cls, url):
        ...
        match = re.match(
            r"^(https?://(?:www\.)?%s\.com/fiction/\d+)/?.*" % cls.domain, url
        )
        if match:
    ...
    def extract(self, url):
        workid = re.match(
            r"^https?://(?:www\.)?%s\.com/fiction/(\d+)/?.*" % self.domain, url
        ).group(1)
        ...
        story = Story(
            ...
            tags=[
                tag.get_text().strip() for tag in soup.select("span.tags a.fiction-tag")
            ],
        )
        ...
        for chapter in soup.select("#chapters tbody tr[data-url]"):
            ...
            story.add(
                Chapter(
                    title=chapter.find("a", href=True).string.strip(),
                    contents=contents,
                    date=updated,
                )
            )
    ...
    def _chapter(self, url, chapterid):
        ...
        if len(author_note) == 1:
            ...
            if (
                "author-note-portlet"
                in soup.find("div", class_="chapter-content").parent.find("div")[
                    "class"
                ]
            ):
